...
for (i in 1:NodesQuantity){
  print(i);
  # Age 
  Avg_Con_Gam <- gam(Avg_Controllability[, i] ~ s(Age, k = 4) + Sex + MotionMeanRelRMS, method="REML", data = Behavioral);
  Avg_Con_Gam_F_P_Matrix[i, c(1, 7)] <- summary(Avg_Con_Gam)$s.table[, 3:4];
  # Mood
  Avg_Con_Gam <- gam(Avg_Controllability[, i] ~ s(Age, k = 4) + Mood + Sex + MotionMeanRelRMS, method = "REML", data = Behavioral);
  Avg_Con_Gam_F_P_Matrix[i, 2] <- as.numeric(summary(Avg_Con_Gam)$p.t[2]);
  Avg_Con_Gam_F_P_Matrix[i, 8] <- as.numeric(summary(Avg_Con_Gam)$p.pv[2]);
  # Psychosis
  Avg_Con_Gam <- gam(Avg_Controllability[, i] ~ s(Age, k = 4) + Psychosis + Sex + MotionMeanRelRMS, method = "REML", data = Behavioral);
  Avg_Con_Gam_F_P_Matrix[i, 3] <- as.numeric(summary(Avg_Con_Gam)$p.t[2]);
  Avg_Con_Gam_F_P_Matrix[i, 9] <- as.numeric(summary(Avg_Con_Gam)$p.pv[2]);
  # Externalizing
  Avg_Con_Gam <- gam(Avg_Controllability[, i] ~ s(Age, k = 4) + Externalizing + Sex + MotionMeanRelRMS, method = "REML", data = Behavioral); 
  Avg_Con_Gam_F_P_Matrix[i, 4] <- as.numeric(summary(Avg_Con_Gam)$p.t[2]);
  Avg_Con_Gam_F_P_Matrix[i, 10] <- as.numeric(summary(Avg_Con_Gam)$p.pv[2]);
  # Phobias
  Avg_Con_Gam <- gam(Avg_Controllability[, i] ~ s(Age, k = 4) + Phobias + Sex + MotionMeanRelRMS, method = "REML", data = Behavioral);
  Avg_Con_Gam_F_P_Matrix[i, 5] <- as.numeric(summary(Avg_Con_Gam)$p.t[2]);
  Avg_Con_Gam_F_P_Matrix[i, 11] <- as.numeric(summary(Avg_Con_Gam)$p.pv[2]);
  # Overall Psycho
  Avg_Con_Gam <- gam(Avg_Controllability[, i] ~ s(Age, k = 4) + OverallPsycho + Sex + MotionMeanRelRMS, method = "REML", data = Behavioral);
  Avg_Con_Gam_F_P_Matrix[i, 6] <- as.numeric(summary(Avg_Con_Gam)$p.t[2]);
  Avg_Con_Gam_F_P_Matrix[i, 12] <- as.numeric(summary(Avg_Con_Gam)$p.pv[2]);
...
for (i in 1:NodesQuantity){
  print(i);
  # Age 
  Mod_Con_Gam <- gam(Mod_Controllability[, i] ~ s(Age, k = 4) + Sex + MotionMeanRelRMS, method = "REML", data = Behavioral);
  Mod_Con_Gam_F_P_Matrix[i, c(1, 7)] <- summary(Mod_Con_Gam)$s.table[, 3:4];
  # Mood
  Mod_Con_Gam <- gam(Mod_Controllability[, i] ~ s(Age, k = 4) + Mood + Sex + MotionMeanRelRMS, method = "REML", data = Behavioral);
  Mod_Con_Gam_F_P_Matrix[i, 2] <- as.numeric(summary(Mod_Con_Gam)$p.t[2]);
  Mod_Con_Gam_F_P_Matrix[i, 8] <- as.numeric(summary(Mod_Con_Gam)$p.pv[2]);
  # Psychosis
  Mod_Con_Gam <- gam(Mod_Controllability[, i] ~ s(Age, k = 4) + Psychosis + Sex + MotionMeanRelRMS, method = "REML", data = Behavioral);
  Mod_Con_Gam_F_P_Matrix[i, 3] <- as.numeric(summary(Mod_Con_Gam)$p.t[2]);
  Mod_Con_Gam_F_P_Matrix[i, 9] <- as.numeric(summary(Mod_Con_Gam)$p.pv[2]);
  # Externalizing
  Mod_Con_Gam <- gam(Mod_Controllability[, i] ~ s(Age, k = 4) + Externalizing + Sex + MotionMeanRelRMS, method = "REML", data = Behavioral);
  Mod_Con_Gam_F_P_Matrix[i, 4] <- as.numeric(summary(Mod_Con_Gam)$p.t[2]);
  Mod_Con_Gam_F_P_Matrix[i, 10] <- as.numeric(summary(Mod_Con_Gam)$p.pv[2]);
  # Phobias
  Mod_Con_Gam <- gam(Mod_Controllability[, i] ~ s(Age, k = 4) + Phobias + Sex + MotionMeanRelRMS, method = "REML", data = Behavioral);
  Mod_Con_Gam_F_P_Matrix[i, 5] <- as.numeric(summary(Mod_Con_Gam)$p.t[2]);
  Mod_Con_Gam_F_P_Matrix[i, 11] <- as.numeric(summary(Mod_Con_Gam)$p.pv[2]);
  # Overall Psycho
  Mod_Con_Gam <- gam(Mod_Controllability[, i] ~ s(Age, k = 4) + OverallPsycho + Sex + MotionMeanRelRMS, method = "REML", data = Behavioral);
  Mod_Con_Gam_F_P_Matrix[i, 6] <- as.numeric(summary(Mod_Con_Gam)$p.t[2]);
  Mod_Con_Gam_F_P_Matrix[i, 12] <- as.numeric(summary(Mod_Con_Gam)$p.pv[2]);
  # resultant vector, in each row, the first 6 elements were f values, and the next 6 elements were p values 
}
for (i in 1:6){
